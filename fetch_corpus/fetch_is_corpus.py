...
TIMEOUT = 30
RETRY = 5
SLEEP = 1.0          # be polite; OpenAlex is free & CC0 (increased from 0.3 to avoid rate limiting)
...
def _session() -> requests.Session:
    s = requests.Session()
    s.headers.update({"User-Agent": "is-field-fetcher/1.0"})
    return s
    ...
    for attempt in range(1, RETRY + 1):
        try:
            ...
            if r.status_code in (403, 429, 500, 502, 503, 504):
                wait_time = min(60, attempt * 5)  # increased backoff
...
def fetch_all_works_for_source(source_id: str, outfile: pathlib.Path) -> None:
    """
    ...
    with outfile.open("w", encoding="utf-8") as f:
        ...
        while True:
            ...
            if total is None:
                ...
                pbar = tqdm(total=total, desc=f"Pulling works for {source_id}", unit="w")
            results = data.get("results", [])
            for rec in results:
...
def main():
    ...
    # 2) Fetch all works for each Source ID (all years)
    all_frames = []
    for jname, sid in name_to_source.items():
        cache_file = CACHE_DIR / f"{jname.replace(' ', '_')}.jsonl"
        if not cache_file.exists():
            fetch_all_works_for_source(sid, cache_file)
        ...
        print(f"[SAVE] {jname}: {len(df):,} rows -> {out_parquet}")
